...
import { isHigerVersion, makeClipboardData } from './common';
  ...
  const getPath = useCallback((cmd: PanelOpenCmd) => {
    switch (cmd) {
      ...
      case 'openAskDoc':
        ...
        if (
          (platform === ClientType.android && isHigerVersion('9.8.6', version)) ||
          (platform === ClientType.ios && isHigerVersion('9.7.14', version)) ||
